...
(defn execute-kcx-command
  [command]
  (try
    (let [conflict-level (dsl/detect-conflict-level command)
      ...
      (if parsed-cmd
        (if (= (:verb parsed-cmd) "proj")
          (orchestrator/handle-proj-command parsed-cmd) ; Handle proj commands directly
